...
public class Wheel {
	...
	private Color tyreColor; // de kleur van de band/velg
	public Color spokeColor; // de kleur van de spaken
	...
	public void draw(Graphics g) {
		...
		// We tekenen eerst de spaken, dan pas de de band 

		double[][] spokePositions = spokePositions();
		g2D.setStroke(spokeStroke);
		g2D.setColor(spokeColor);
		for (double[] coordinate : spokePositions) {
			g.drawLine((int) Math.round(x), (int) Math.round(y), (int) Math.round(coordinate[0]),
					(int) Math.round(coordinate[1]));
		}

		g2D.setColor(tyreColor);
		g2D.setStroke(tyreStroke);
		g2D.drawOval((int) Math.round(x - radius), (int) Math.round(y - radius), (int) Math.round(2 * radius),
